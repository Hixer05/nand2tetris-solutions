        ...
        { // store
          ...
          *pos = pos_l;
          return 0;
            ...
            pos_l
                = hash2 (pos_l, i + 1)%size; // pos + 0^2 = pos; it'd be a useless cycle
    }
  return -1;
}

[[nodiscard]] int
searchHashMap (char **const hashTable, char const *const key, const size_t size,
               size_t *const pos)
{
    size_t pos_l = hash1(key)%size;
    for(register int i = 0; i < size; i++){
        if(hashTable[pos_l][0] == '\0') // not found
            return -1;
        else if(!strcmp(hashTable[pos_l], key)){ // found
            *pos = pos_l;
            return 0;
        } else // occupied
